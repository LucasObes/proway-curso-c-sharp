...
namespace WindowsFormsExemplos.Repositorios
{
    internal class CarrinhoRepositorio
    {
        ...
        public int AdicionarItemCarrinho(Carrinho carrinho)
        {
            ...
            comando.CommandText = @"INSERT INTO carrinho (id_pedido, id_produto, quantidade) OUTPUT INSERTED.ID
                                  VALUES (@ID_PEDIDO, @ID_PRODUTO, @QUANTIDADE)";
            // OUTPUT INSERTED.ID: pega os IDs que já estão cadastrados no banco
        ...
        internal IList<Carrinho> ObterItensPorIdPedido(int idPedido)
        {
            ...
            comando.CommandText = @"SELECT 
                                  ...
                                  FROM carrinho INNER JOIN produtos ON (carrinho.id_produto = produtos.id)
        ...
        private Carrinho ConstruirItemCarrinhoDoRegistro(DataRow registro)
        {
            ...
            carrinho.Produto.Id = Convert.ToInt32(registro["produto_id"]);
